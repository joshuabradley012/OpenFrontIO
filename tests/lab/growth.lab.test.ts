...
const OUT =
  "/private/tmp/claude-501/-Users-josh-Code-openfront/f46e4d3b-aecb-4e40-bb41-205a4bfbadb7/scratchpad/";
...
test("opening curves", async () => {
  const policies: [
    string,
    (troops: number, cap: number, tick: number) => number,
  ][] = [
    ...
    [
      "all-in every 5 s (keep 15% cap)",
      (t, c, k) => (k % 50 === 0 ? Math.max(0, t - c * 0.15) : 0),
    ],
    ["3% every 1 s", (t, c, k) => (k % 10 === 0 ? t * 0.03 : 0)],
    ["7% every 1 s", (t, c, k) => (k % 10 === 0 ? t * 0.07 : 0)],
    [
      "10% for 30 s, then 5%",
      (t, c, k) => (k % 10 === 0 ? t * (k < 300 ? 0.1 : 0.05) : 0),
    ],
    [
      "15% for 20 s, then 5%",
      (t, c, k) => (k % 10 === 0 ? t * (k < 200 ? 0.15 : 0.05) : 0),
    ],
  ...
  for (const [name, pol] of policies) {
    ...
    for (let dy = -3; dy <= 3; dy++)
      for (let dx = -3; dx <= 3; dx++) me.conquer(game.ref(100 + dx, 100 + dy));
    ...
    for (let k = 0; k <= 1200; k++) {
      ...
      if (send >= 100)
        game.addExecution(
          new AttackExecution(send, me, game.terraNullius().id()),
        );
      game.executeNextTick();
      if (k % 50 === 0)
        rows.push(
          `${k / 10},${me.numTilesOwned()},${Math.round(me.troops())},${Math.round(me.troops() + me.outgoingAttacks().reduce((a, x) => a + x.troops(), 0))}`,
        );
    }
    out.push(`== ${name}\n` + rows.join("\n"));
  }
