...
describe("estimateAttack", () => {
  ...
  test("calibration inputs: lossScale scales the loss per tile, speedScale the tiles per tick, extraDefenderTroops the defence", async () => {
    const { game, attacker, defender } = await twoPlayers(600_000, 60_000);
    const base = estimateAttack(game, attacker, defender, 100_000, { horizonTicks: 200 });
    const dear = estimateAttack(game, attacker, defender, 100_000, { horizonTicks: 200, lossScale: 2 });
    expect(dear.attackerLoss / Math.max(1, dear.tilesTaken)).toBeGreaterThan((base.attackerLoss / Math.max(1, base.tilesTaken)) * 1.8);
    // speed: a wave too big to be loss-limited inside a short horizon
    const fast = estimateAttack(game, attacker, defender, 300_000, { horizonTicks: 100 });
    const slow = estimateAttack(game, attacker, defender, 300_000, { horizonTicks: 100, speedScale: 0.5 });
    expect(slow.tilesTaken).toBeLessThan(fast.tilesTaken * 0.7);
    expect(slow.tilesTaken).toBeGreaterThan(0);
    const allied = estimateAttack(game, attacker, defender, 300_000, { horizonTicks: 100, extraDefenderTroops: 120_000 });
    expect(allied.attackerLoss / Math.max(1, allied.tilesTaken)).toBeGreaterThan(fast.attackerLoss / Math.max(1, fast.tilesTaken));
    expect(allied.troopsLeft).toBeLessThan(fast.troopsLeft);
    expect(estimateAttack(game, attacker, defender, 100_000, { horizonTicks: 200, lossScale: 1, speedScale: 1, extraDefenderTroops: 0 })).toEqual(base);
  });
