...
async function alliedWeakAlly(lapseToAttack: boolean, rivalTroops: number, bot: Partial<PlaybookParams> = {}, big = false) {
  const h = await playbookSetup({
    map: "big_plains", spawn: centre(ME), tiles: ME, troops: 300_000, bot: { ...HOME, lapseToAttack, ...bot },
    rivals: [
      { name: "A", type: PlayerType.Human, at: centre(ALLY), tiles: ALLY, troops: 60_000 },
      { name: "R1", type: PlayerType.Human, at: centre(big ? NORTH_BIG : NORTH), tiles: big ? NORTH_BIG : NORTH, troops: rivalTroops },
      { name: "R2", type: PlayerType.Human, at: centre(big ? SOUTH_BIG : SOUTH), tiles: big ? SOUTH_BIG : SOUTH, troops: rivalTroops },
    ],
...
const ratio = (h: PlaybookHarness, name: string) => h.rival(name).troops() / h.me.troops();
...
describe("lapseToAttack", () => {
  ...
  test("on: the ally is the best war on the board → it lapses, logged with its score, the flag fires", async () => {
    ...
    const line = h.log.find((l) => /let alliance lapse to attack A \(score \d+\.\d, \d+k vs our \d+k\)/.test(l));
    ...
    expect(h.until(() => h.me.outgoingAttacks().some((x) => x.target() === a), 60)).toBe(true);
  });

  test("on: a stronger unfriendly neighbour (> 0.6× our troops) on the border keeps the alliance", async () => {
    const { h, a, al, expiresAt } = await alliedWeakAlly(true, 450_000, {}, true);
    h.step(1);
    ...
    expect(h.log.some((l) => /let alliance lapse to attack/.test(l))).toBe(false);
    expect(h.bot.fired.get("lapseToAttack")).toBeUndefined();
    expect(h.me.isFriendly(a)).toBe(true);
  });
  ...
  test("on: an ally the scorer refuses (under the ratio gate) is renewed", async () => {
    ...
    const { h, a, al, expiresAt } = await alliedWeakAlly(true, 60_000, { fightMaxShare: 0.28, fightAbove: 0 });
    h.step(1);
