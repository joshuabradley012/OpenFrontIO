...
export class Economy {
  ...
  build(ticks: number): void {
    ...
    if (cityUnits.length >= 1 && ticks >= 900 && gold >= cost(UnitType.DefensePost) && me.unitsOwned(UnitType.DefensePost) < 6) { // a threat post never waits for city 2 (30-game lab: +8 % land, same survival)
      ...
      // #6 (`campaigns`): the target of a campaign in prepare gets its post right after the expiring allies
      const prep = this.ctx.p.campaigns && this.military.prepTarget !== null && rivals.includes(this.military.prepTarget) ? [this.military.prepTarget] : [];
      const threat = [...expiring, ...prep, ...pre, ...rivals].find((r) => ticks - (this.postFailed_.get(r) ?? -1e9) > 600 && (r.troops() >= me.troops() * 0.5 || expiring.includes(r) || prep.includes(r) || (r.type() === PlayerType.Nation && me.troops() > r.troops() * 3)) && !this.q.postFacing(r));
      if (threat) { if (prep.length > 0 && threat === prep[0]) this.ctx.fire("campaigns"); if (pre.length > 0 && threat === pre[0] && !expiring.includes(threat) && !(threat.troops() >= me.troops() * 0.5 || (threat.type() === PlayerType.Nation && me.troops() > threat.troops() * 3))) this.ctx.fire("threatMap"); const tile = this.defensePostTile(threat); if (tile !== null && this.tryBuild(UnitType.DefensePost, tile)) return; this.postFailed_.set(threat, ticks); this.ctx.log(`t${ticks} post vs ${threat.name()} FAILED (${tile === null ? "no tile" : "canBuild"})`); }
