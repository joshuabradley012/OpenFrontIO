...
// Removed 2026-08-31 (Josh: "probably isn't good, remove it"): tribeBorders + Military.tribeRival — both
//   single-seed smokes were bad and it never got its A/B; last commit with the code: c38fc2020.
...
export interface PlaybookParams {
  ...
  // boatOceanUntil (v3's all-or-nothing ocean window) was REMOVED in v6: the escalating boatSailMin/boatSailRampTicks budget below replaces it (Josh: "close boats very early, then further and further attempts before warships are everywhere")
  boatSailMin: number; // boatOpening v6: the opening extras' sail budget at tick 0 (tiles of true water path; int) — the budget grows linearly to the full BOAT_MAX_PATH (250) over boatSailRampTicks: maxSail(t) = boatSailMin + (250 − boatSailMin) × clamp(t / boatSailRampTicks, 0, 1)
  boatSailRampTicks: number; // boatOpening v6: ticks until the sail budget reaches the full BOAT_MAX_PATH (int) — sized to end around when enemy warships appear (first enemy warship t1489–t2205 across the 6 measurement games, both difficulties; a long crossing after that is sunk, escorted or not)
  boatOceanBonus: number; // boatOpening v3/v6: score multiplier on a new-landmass candidate whose sail exceeds BOAT_MAX_PATH.early (80) — the trans-ocean continent grab, now inside the growing v6 sail budget instead of the removed boatOceanUntil window
  ...
  boatEatRateHard: number; // boatOpening v6: boatEatRate for Hard/Impossible games — the difficulty knob Josh's Hard sessions asked for (landings on shores the tribes ate first). Measured the v3 way (3 Hard + 3 Medium lab games, tiles/tick per frontier-contact tile, t100–3000): Hard/Medium ratio 1.013 pooled (tribes 1.012, nations 1.031) — per-contact-tile expansion is nearly difficulty-independent, so the default is 0.0245 × 1.013; the landing-eaten re-anchor is what actually attacks the Hard symptom, and the CMA can move this knob later
  ...
  falloutPatience: boolean; // Josh's "wait for irradiated land to cool" — but engine fallout NEVER cools: one bit per tile (GameMap bit 13), cleared only by conquest (GameImpl.conquer) or flood, and the ~5× penalty (Config.falloutDefenseModifier, 5 − 2 × GLOBAL fallout share) never varies with tile age. So the wait is for the BOMBS, not the isotopes: defer takeFallout's expand while the bordering fallout basins are still growing (a nuke landed < falloutCoolTicks ago — land cleared at ~5× mag can be re-nuked away), UNLESS a living hostile non-bot borders a basin (front-line fallout is always taken: no defender fights back on unowned tiles, it advances the border and denies the human reclaim — nations/bots never take fallout at all, AiAttackBehavior skips it). default off until the 30-game Medium A/B
  falloutCoolTicks: number; // falloutPatience quiet window: the bordering basins must not have grown for this many ticks before we pay the ~5× mag (silo cooldown is 90 ticks — an active bombardment refires well inside 300)
  ...
  thinGuard: boolean; // Josh (Hard GUI, 2026-08-31): "it creates really thin parts of its territory, open to getting cut off and giving a chunk to tribes". Every 100 ticks scan our sampled border for pinches — a run of ≤ thinWidth of our tiles with non-owned LAND on both ends (water does not count: a strait is not land-cuttable). On the narrowest pinch: a side facing free land gets an immediate expand click at the contested share; a side facing a tribe marks that tribe first in harvestBots' order (through the existing budgets); a rival on both sides requests a defense post at the pinch through the existing ≤8-post budget. Logs `THIN (x,y) width~w`. Additionally, while a tribe wave is unfinished, follow-ups go at half botFollowUpTicks (hard0 transcripts: a half-eaten tribe leaves the salient that gets cut). Default off until the full-game A/B
  thinWidth: number; // thinGuard: a corridor of ours this many tiles wide or less, with non-owned land on both sides, is a pinch (int)
  postStandoff: number; // Josh (Hard GUI, 2026-08-31): "the bot builds defense posts too close to enemy borders — it uses the center of the building on their edge, not the edge of the defense radius like a human would". The engine grants the 5×/3× defense bonus to any conquered tile with a defender post within defensePostRange (30, EUCLIDEAN — AttackExecution.attackLogicInput/hasUnitNearby), so a post ON the border wastes the interior half of its circle and falls in the first push. Every post site pick (attack-landing + threat posts via Economy.defensePostTile, boatDefense landing + thinGuard pinch posts via landingPostTile — they share Economy.standoffPostTile) now steps ~this many tiles back from the border contact toward our interior: the buildable ring candidate ~postStandoff from the contact midpoint, at least (standoff − 2) from every scored border tile (keeps it on the border-normal, off the border itself), that covers the most border tiles within the radius; the ring shrinks by 4 toward the contact when the interior is too small (a 3-tile island still builds, at the contact). Situation.postFacing widens its scan to keep seeing the rival past the standoff. Default ON as a correction (a bug-shaped misplacement, no flag); 0 restores the old contact-tile geometry byte-for-byte (the parity pin), and the CMA can tune it (int)
...
export const DEFAULT_PLAYBOOK: PlaybookParams = {
  ...
  boatSailMin: 50, // v6: close boats first (Josh) — at t0 the extras stay within ~50 sailed tiles
  boatSailRampTicks: 2000, // v6: full 250-tile budget from t2000 on — warships that would sink a long crossing appear t1489–t2205 (6 measurement games), so the longest attempts run right as the ocean closes
  boatOceanBonus: 1.3,
  ...
  boatEatRateHard: 0.0248, // v6 measured: 0.0245 × 1.013, the pooled Hard/Medium ratio of the same instrument over 3 seeds each (tiles/tick per frontier-contact tile, tribes+nations, t100–3000 africa)
  ...
  falloutPatience: false,
  falloutCoolTicks: 300, // 30 s of quiet
  ...
  bdCoastRange: 30,
  bdBeachheadMax: 800,
  thinGuard: false, // default off until the full-game A/B (Hard tribe-trap follow-up)
  thinWidth: 6,
  postStandoff: 28, // defense radius (Config.defensePostRange 30) − 2: the radius edge reaches the enemy border; 0 = the old contact-tile placement
