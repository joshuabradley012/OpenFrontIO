...
export interface PlaybookParams {
  ...
  boatsWaterPath: boolean; // every boat rule (earlyBoat, huntBotsByBoat, sendBoat, seaExpansion, seaInvasion, finishByBoat) ranks a candidate by the length of the water path the transport will sail (Military.waterPath: a breadth-first fill over water tiles from our sampled shore, ≤ 40k tiles, cached 100 ticks) instead of the straight-line distance, and refuses a landing whose path exceeds Military.BOAT_MAX_PATH for that rule (early 80, tribe/island 150, sea 200, finish 250, invade 300); composes with boatsNearest on or off; default off until the 30-game Medium A/B
  boatsAfterCoast: boolean; // no early boat and no tribe boat while free land is still reachable by land on our own landmass (sit.wilderness or Situation.freeLandReachable) unless we start on a small landmass (islandMaxTiles): expand to the coast first, then boat; seaExpansion keeps its own wilderness gate, finishByBoat and seaInvasion are unchanged; default off until the 30-game Medium A/B
