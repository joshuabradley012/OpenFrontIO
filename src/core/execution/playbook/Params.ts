...
export interface PlaybookParams {
  ...
  tribeBorders: boolean; // Josh (Hard GUI, 2026-08-31, "early game tribe taking traps"): harvestBots eats the weakest bordering tribe first, so the tribes that matter — the ones ALSO bordering a non-ally rival — are left standing until the rival takes them and appears on our doorstep. Under the flag only the ORDER changes (same sizing, gates, concurrency): tribes whose sampled border touches a rival are eaten first (weakest first among them; equal troops tie-break toward the tribe whose border is most ours already — eating it shortens our exposed frontier most), turning a soft tribe frontier into a real border before the rival takes it. Border check sampled every 3rd tribe-border tile, cached 100 ticks. Logs `TRIBE PRIORITY <name> (borders <rival>)` when the pick differs from the plain order. Default off until the full-game A/B
...
export const DEFAULT_PLAYBOOK: PlaybookParams = {
  ...
  bdCoastRange: 30,
  bdBeachheadMax: 800,
  tribeBorders: false, // default off until the full-game A/B (Hard tribe-trap follow-up)
