...
//
// Calibration (opportunity #4): the replay is exact for one attack against a passive defender but blind to merges,
// allies piling in, posts built mid-war and the target's other wars. `lossScale` / `speedScale` are per-defender-class
// multipliers fitted offline from the EST/ACT log pairs (scripts/lab/calibrate.py) and `extraDefenderTroops` is what
// the target's allies could add (Rivals.nationWouldSend). All three default to the uncalibrated replay.
//
...
export interface EstimateOptions {
  ...
  lossScale?: number; // multiplier on the attacker's loss per tile (calibration, default 1)
  speedScale?: number; // multiplier on tiles per tick (calibration, default 1)
  extraDefenderTroops?: number; // troops added to the defender's virtual pool (allies that could pile in), default 0
}
...
export function estimateAttack(
  ...
  const lossScale = opts.lossScale ?? 1;
  const speedScale = opts.speedScale ?? 1;
  const extra = defender.troops() > 0 ? Math.max(0, Math.floor(opts.extraDefenderTroops ?? 0)) : 0; // an empty defender is handed over; no probes possible
  const map = mg.map();
  ...
  // the attackLogic probes below must see the live ratio, so the probe base is the defender's real troops; the
  // virtual pool the wave fights through starts higher by the allies' possible pile-in
  const d0 = defender.troops();
  let d = d0 + extra;
  ...
  while (simTick < horizon) {
    ...
    let perTick = within(((5 * a) / d) * 2, 0.01, 0.5) * (border.size + random.nextInt(0, 5)) * 3 * speedScale;
    while (perTick > 0) {
      ...
      const attackerLoss = (d > 0 ? 0.6 * within(d / a, 0.6, 2) * c.x + density * c.y : 0) * lossScale;
