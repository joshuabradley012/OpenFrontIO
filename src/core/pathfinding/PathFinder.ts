...
class WaterPathMemo implements PathFinder<TileRef> {
  /** Live cache budget. ~64 % of a 170-minute game's queries hit at this size
   *  (measured 2026-08-31); the whole store stays small enough for the client
   *  worker, where the sim also runs. */
  ...
  findPath(from: TileRef | TileRef[], to: TileRef): TileRef[] | null {
    ...
    while (this.liveBytes > WaterPathMemo.MAX_BYTES) {
      ...
      this.liveBytes -= oldest === null || oldest === undefined ? 16 : oldest.byteLength;
    }
    return path;
  }
}
